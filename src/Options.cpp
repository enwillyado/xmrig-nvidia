...
bool Options::save()
{
	...
	uv_fs_t req;
	const int fd = uv_fs_open(uv_default_loop(), &req, m_configName.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644,
	                          nullptr);
	if(fd < 0)
	{
		return false;
	...
	doc.AddMember("log-file", (0 < m_logFile.size()) ? rapidjson::Value(rapidjson::StringRef(
	                  algoName())).Move() : rapidjson::Value(rapidjson::kNullType).Move(), allocator);
...
#   endif
	...
	{
		const GpuThread* thread = m_threads[i];
	...
	{
		const Url & url = m_pools[i];
		rapidjson::Value obj(rapidjson::kObjectType);
		snprintf(tmp, sizeof(tmp) - 1, "%s:%d", url.host(), url.port());

		obj.AddMember("url",       rapidjson::StringRef(tmp), allocator);
		obj.AddMember("user",      rapidjson::StringRef(url.user().c_str()), allocator);
		obj.AddMember("pass",      rapidjson::StringRef(url.password().c_str()), allocator);
	...
	api.AddMember("access-token", (0 < m_apiToken.size()) ? rapidjson::Value(rapidjson::StringRef(
	                  m_apiToken.c_str())).Move() : rapidjson::Value(rapidjson::kNullType).Move(), allocator);
	api.AddMember("worker-id", (0 < m_apiWorkerId.size()) ? rapidjson::Value(rapidjson::StringRef(
	                  m_apiWorkerId.c_str())).Move() : rapidjson::Value(rapidjson::kNullType).Move(), allocator);
	...
	m_threads(0)
{
	...
	if(m_threads.empty() && !m_cudaCLI.setup(m_threads))
	{
		...
		for(size_t i = 0; i < m_threads.size(); ++i)
		{
			GpuThread* const thread = m_threads[i];
...
bool Options::parseArg(int key, const std::string & arg)
{
	...
	case 1201: /* --cuda-bfactor */
		m_cudaCLI.parseBFactor(arg.c_str());
		break;

	case 1202: /* --cuda-bsleep */
		m_cudaCLI.parseBSleep(arg.c_str());
		break;

	case 1203: /* --cuda-devices */
		m_cudaCLI.parseDevices(arg.c_str());
		break;

	case 1204: /* --cuda-launch */
		m_cudaCLI.parseLaunch(arg.c_str());
		break;

	case 1205: /* --cuda-affinity */
		m_cudaCLI.parseAffinity(arg.c_str());
		break;
...
void Options::parseConfig(const std::string & fileName)
{
	...
	if(pools.IsArray())
	{
		for(size_t i = 0; i < threads.GetArray().Size(); ++i)
		{
			const rapidjson::Value & value = threads.GetArray()[i];
			if(!value.IsObject())
			{
				continue;
			}
