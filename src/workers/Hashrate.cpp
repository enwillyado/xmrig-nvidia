...
inline std::string format(double h, char* buf, size_t size)
{
...
void Hashrate::print()
{
	...
	if(Options::i()->colors())
	{
		/*
		LOG_INFO(
			"\x1B[01;37mspeed\x1B[0m 10s/60s/15m \x1B[01;36m%s\x1B[0m \x1B[22;36m%s %s \x1B[01;36mH/s\x1B[0m max: \x1B[01;36m%s H/s",
	...
	{
		LOG_INFO("speed 10s/60s/15m " << format(calc(ShortInterval), num1, sizeof(num1)) << " " <<
		         format(calc(MediumInterval), num2, sizeof(num2)) << " " <<
		         format(calc(LargeInterval), num3, sizeof(num3)) << "H/s max: " <<
		         format(m_highest, num4, sizeof(num4)) << " H/s");
	}
}
...
void Hashrate::print(size_t threadId, int gpuId)
{
	...
	{
		/*
		LOG_INFO("\x1B[01;37mGPU %d\x1B[0m 10s/60s/15m \x1B[01;36m%s\x1B[0m \x1B[22;36m%s %s \x1B[01;36mH/s",
		gpuId,
	...
	{
		LOG_INFO("GPU #" << gpuId << " speed 10s/60s/15m " <<
		         format(calc(threadId, ShortInterval),  num1, sizeof(num1)) << " " <<
		         format(calc(threadId, MediumInterval), num2, sizeof(num2)) << " " <<
		         format(calc(threadId, LargeInterval),  num3, sizeof(num3)) << " H/s");
	}
}
